...
class ActDeact extends Engine\Base {
	...
	public function initialize() {
		...
		register_activation_hook( C_TEXTDOMAIN . '/' . C_TEXTDOMAIN . '.php', array( __CLASS__, 'activate' ) );
		register_deactivation_hook( C_TEXTDOMAIN . '/' . C_TEXTDOMAIN . '.php', array( __CLASS__, 'deactivate' ) );
	...
	public static function upgrade_procedure() {
		if ( is_admin() ) {
			$version = get_option( 'coachs-version' );
			if ( version_compare( C_VERSION, $version, '>' ) ) {
				update_option( 'coachs-version', C_VERSION );
				delete_option( C_TEXTDOMAIN . '_fake-meta' );
			}
		}
	}
